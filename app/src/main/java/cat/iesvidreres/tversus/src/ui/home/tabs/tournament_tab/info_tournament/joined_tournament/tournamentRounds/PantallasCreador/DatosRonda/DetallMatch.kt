...
class DetallMatch : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        equipo1Layout.setOnClickListener {
            Toast.makeText(this, "Equipo1 Ganador", Toast.LENGTH_SHORT).show()
            match?.ganadorSeleccionado = true
            if (match != null) {
                val intent = Intent()
                intent.putExtra("equipo", equip1)
                setResult(Activity.RESULT_OK, intent)
                finish()
            }
        }

        equipo2Layout.setOnClickListener {
            Toast.makeText(this, "Equipo2 Ganador", Toast.LENGTH_SHORT).show()
            match?.ganadorSeleccionado = true
            if (match != null) {
                val intent = Intent()
                intent.putExtra("Ganador",true)
                intent.putExtra("equipo", equip2)
                setResult(Activity.RESULT_OK, intent)
                finish()
            }
        }
